...
require_once('../../DAL/DAL.class.php');
  ...
  class filtering{
    ...
    public function getSales(){
        $sql = "SELECT * FROM `products` where product_price!=product_final_price AND   status=1 LIMIT 9 ORDER BY id DESC";
        $db = new DAL();
        $rows = $db->getData($sql);
        return $rows;
    }
    ...
    public function getAllSales(){
      $sql = "SELECT * FROM `products` where product_price!=product_final_price AND   status=1 ORDER BY id DESC";
      $db = new DAL();
      $rows = $db->getData($sql);
      return $rows;
  }
